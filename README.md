...
Если прокси со временем перестает принимать клиентов, перед перезапуском можно снять состояние процесса:

```
chmod +x diagnose-vk-turn-proxy.sh
./diagnose-vk-turn-proxy.sh
```

Скрипт сам находит процесс `vk-turn-proxy`, маскирует `-wrap-key` в выводе и показывает количество fd, UDP-сокетов, потоков, CPU/RAM и список UDP-сокетов процесса. Если бинарник запущен под другим именем, передайте его первым аргументом:

```
./diagnose-vk-turn-proxy.sh ./vk-turn-proxy
```

#### Диапазон внешних портов

Иногда один публичный UDP-порт, например `56000`, может оказаться заблокирован на стороне TURN/VK-серверов, которые доставляют трафик к прокси от лица клиента. В таком случае можно публиковать диапазон внешних портов и перенаправлять их на один внутренний порт прокси, например `30000-30100 -> 56000`.

Такой режим полезен для обхода блокировки конкретного порта, но у него есть эксплуатационные риски: больше открытых UDP-портов получает фоновые сканы и мусорный трафик, а conntrack/NAT держит больше UDP-состояний. Если прокси со временем начинает массово писать `Handshake failed`, полезно сравнить поведение с одним внешним портом и с диапазоном, а также посмотреть состояние conntrack:
